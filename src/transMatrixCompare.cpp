...
// Reads a linear-system directory in the same layout as jsonSolve (a
// BlackoilMatrix.mtx / transAdj.mtx pair, see help_detect() in
// io/inputRead.hpp), and instead of solving the system, compares every
// transmissibility value against each of the 9 entries of the
// corresponding 3x3 off-diagonal block in the system matrix. The
// (transmissibility, matrix-entry) pairs are written to a CSV file, and a
// bundled Python/matplotlib script is invoked to render the comparison as
// a plot.
//
// If the directory also holds a porosity.csv (as written by the
// writePorosity tool, columns "cell,i,j,k,poro", cell numbered 0..N-1
// matching the row/column numbering of the matrices), a second comparison
// is produced dividing each row's transmissibility by that row's (i.e.
// that cell's) porosity, so both the raw transmissibility and the
// porosity-scaled transmissibility get plotted against the matrix
// entries: four plots in total.
...
// Matrix block sizes matching jsonSolve.cpp: a 3x3-block system matrix
// (3-phase black-oil) and a scalar (1x1) transmissibility/connectivity
...
void printUsage(const char* prog)
{
    std::cerr << "Compares transmissibility values against the corresponding off-diagonal\n"
	      << "block entries of a black-oil system matrix and plots the result.\n\n"
	      << "Usage: " << prog << " matrix/dir/path [outputPrefix]\n\n"
	      << "matrix/dir/path must contain the same two files jsonSolve reads its\n"
	      << "system from:\n"
	      << "  *BlackoilMatrix.mtx: 3x3-block system matrix.\n"
...
int main(int argc, char** argv)
{
    ...
    csv << "i,j,trans,m00,m01,m02,m10,m11,m12,m20,m21,m22\n";
    ...
    if (havePoro) {
        ...
        poroCsv << "i,j,trans,m00,m01,m02,m10,m11,m12,m20,m21,m22\n";
    }
