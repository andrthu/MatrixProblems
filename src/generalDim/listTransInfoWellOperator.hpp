...
#include "../solveParallel/filterFunctions.hpp"
...
void gen_dim_list_trans_info(int argc, char** argv)
{
    ...
    //typedef GhostLastMatrixAdapter<Mat3,Vec,Vec,Comm> GLO;
    typedef Opm::WellModelGhostLastMatrixAdapter<Mat3,Vec,Vec,true> GLO;
    ...
    std::vector<Vec> rhs;
    std::vector<Vec> wgts;
    std::vector<std::vector<StandardWell>> wellMod;
    std::vector<std::vector<MultiSegmentWell>> msWellMod;
	...
	if ( boost::algorithm::ends_with( systemDirs[i], ".json") ) {
	    ...
	    readWellMatOnly(wellsFake, systemDirs[i], rank);
	    ...
	    zoltanPartitionFunction(part, trans, wellsFake, cc, DR, rs, 10);

	    mats.push_back(trans);

	    Mat3 A_loc;
	    Vec rhs_loc, impes;
	    std::vector<StandardWell> wells;
	    std::vector<MultiSegmentWell> msWells;
	    ...
	    wellMod.push_back(wells);
	    msWellMod.push_back(msWells);
