...
void gen_dim_coarse_part_info(int argc, char** argv)
{
	...
	if ( boost::algorithm::ends_with( systemDirs[i], ".json") ) {
	    ...
	    std::vector<double> numW = {0.95, 0.9, 0.88, 0.85, 0.82, 0.8, 0.75, 0.7, 0.65, 0.6};

	    for (double nw : numW ) {
	        std::vector<int> rs;
		TransWellGraph twg(trans, wells, rs, 0, 1, rank==0);

		if (rank == 0) {std::cout << "W: " << nw<< " ";}
		double threshold = twg.sortTransFindThreshold(nw);
		twg.coarsenGraphMaxNodeSize(threshold, 10000000, rank);
	    }
	    /*
