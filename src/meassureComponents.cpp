...
int main(int argc, char** argv)
{
    ...
    AMGILU0 amg1(linOp, criterion, smoother1Args, comm);

    if (rank == 0) {std::cout << std::endl;}
    AMGJac amg2(linOp, criterion, smoother2Args, comm);
    ...
    */
    ...
    cc.barrier();
    //multipleMeassureComm(cc, info, local2global.size());
    multipleMinLoopTimeComm(cc, comm, rhs_loc);
    ...
    cc.barrier();
    if (rank == 0) {std::cout << std::endl;}
    multipleMinLoopTimeSpMV(cc, ghostLastOp, rhs_loc, numTimeOp, true);
    
    //ILU apply
    cc.barrier();
    if (rank == 0) {std::cout << std::endl;}
    multipleMinLoopTimePre(cc, *preCon, rhs_loc, numTimeOp);

    //GL ILU apply
    cc.barrier();
    if (rank == 0) {std::cout << std::endl;}
    multipleMinLoopTimePre(cc, *GLpre, rhs_loc, numTimeOp, true, false);
    
    //time scalar product
    cc.barrier();
    if (rank == 0) {std::cout << std::endl;}
    //multipleTimeScalarProduct(cc, info, rhs_loc, numTimeOp);
    //multipleAverageTimeScalarProduct(cc, sp, rhs_loc, numTimeOp,false);
    multipleMinLoopTimeSP(cc, sp, rhs_loc, numTimeOp, false);

    //Ghost last timing
    cc.barrier();
    if (rank == 0) {std::cout << std::endl;}
    multipleMinLoopTimeSP(cc, ghostLastSp, rhs_loc, numTimeOp, true);

    // one BiCGSTAB
    cc.barrier();
    if (rank == 0) {std::cout << std::endl;}
    multipleMinLoopTimeSolver(cc, oneIterBicg, rhs_loc, numTimeOp);

    //One GL BiCGSTAB 
    cc.barrier();
    if (rank == 0) {std::cout << std::endl;}
    multipleMinLoopTimeSolver(cc, ghostLastSolver, rhs_loc, numTimeOp, true);

    // one BiCGSTAB
    cc.barrier();
    if (rank == 0) {std::cout << std::endl;}
    multipleMinLoopTimeSolver(cc, hundredIterBiCG, rhs_loc, numTimeOp, false, true);

    //One GL BiCGSTAB 
    cc.barrier();
    if (rank == 0) {std::cout << std::endl;}
    multipleMinLoopTimeSolver(cc, hundredIterGLSolver, rhs_loc, numTimeOp, true, true);
